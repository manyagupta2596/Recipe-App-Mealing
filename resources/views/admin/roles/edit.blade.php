...
<form action="{{ route('admin.roles.update', $role) }}" method="POST" class="p-4 space-y-4">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-6">
        ...
        <input type="text" name="name" id="name" value="{{ old('name', $role->name) }}" class="border-1 border-gray-100 shadow bg-opacity-20 rounded-lg placeholder-gray-500 w-full lg:w-60 focus:outline-none focus:ring-1 focus:border-green-500 focus:ring-green-500 dark:bg-gray-900 dark:border-transparent dark:text-gray-200">
    </div>
    <div class="">
