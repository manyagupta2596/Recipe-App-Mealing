        ...
        @if (! is_null($recipes))
            @for ($i = 0; $i < 7; $i++)
                <div class="rounded shadow bg-white dark:bg-gray-700">
                    ...
                    <div>
                        @if( $recipes[$i]->getMedia()->count() > 0)
                            <img src="{{ $recipes[$i]->getFirstMediaUrl() }}" class="w-full h-48 object-cover">
                        @else
                    ...
                    <div class="p-2">
                        ...
                        <p class="text-sm text-yellow-400 mb-3" title="{{ $recipes[$i]->avg_rating > 0 ? $recipes[$i]->avg_rating : '0' }}">
                            @for ($x = 0; $x < 5; $x++)
                                @if (floor($recipes[$i]->avg_rating)-$x >= 1)
                                    <i class="fas fa-star"></i>
                                @elseif ($recipes[$i]->avg_rating-$x > 0)
                        ...
                        <p class="mb-3 dark:text-gray-200">
                            Serves: {{ $recipes[$i]->servings }}
                        </p>
                        <input type="hidden" name="{{ $days[$i] }}" wire:model="recipeIds.{{ $i }}">
